...
create_source_bin (guint index, gchar * uri)
{
  ...
  // Handle camera input (e.g. /dev/video0)
  if (g_str_has_prefix (uri, "/dev/video")) {
    ...
    GstElement *videoconvert = gst_element_factory_make ("videoconvert", "camera-convert");
    GstElement *nvvideoconvert = gst_element_factory_make ("nvvideoconvert", "nv-camera-convert");

    if (!src || !capsfilter || !videoconvert || !nvvideoconvert) {
      g_printerr ("Camera source elements could not be created\n");
      return NULL;
    }

    g_object_set (G_OBJECT (src), "device", uri, NULL);


    // Set desired camera format
    GstCaps *caps = gst_caps_from_string ("video/x-raw, width=1280, height=800, framerate=30/1");
    g_object_set (G_OBJECT (capsfilter), "caps", caps, NULL);
    gst_caps_unref (caps);

    gst_bin_add_many (GST_BIN (bin), src, capsfilter, videoconvert, nvvideoconvert, NULL);
    if (!gst_element_link_many (src, capsfilter, videoconvert, nvvideoconvert, NULL)) {
      g_printerr ("Failed to link camera elements\n");
      return NULL;
    }
    ...
    if (!gst_element_add_pad (bin, gst_ghost_pad_new_no_target ("src", GST_PAD_SRC))) {
      g_printerr ("Failed to add ghost pad in URI source bin\n");
      return NULL;
    }
  }
